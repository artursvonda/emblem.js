
{
  ...
  // TODO: This needs to be changed according to Ember version. 
  // It was changed from bindAttr to bind-attr in Ember 1.0.0-rc.8 (August 28, 2013)
  var BIND_ATTR_TAG = 'bind-attr';
    ...
    if (classes.length) {
      ...
      if (isClassAttr && fullAttributes[i].length === 4) {
        if (fullAttributes[i][2].type == 'mustache') {
          ...
          params = [new AST.IdNode([{ part: BIND_ATTR_TAG}])].concat(mustacheNode.params);
...
{ 
  ...
  var params = [new AST.IdNode([{part: BIND_ATTR_TAG}])];
